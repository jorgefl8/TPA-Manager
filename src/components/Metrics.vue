            ...
            <Fieldset :legend="slotProps.data[0]" :toggleable="true" :collapsed="collapsed[slotProps.index]" class="col-12" @toggle="collapsed[slotProps.index] = !collapsed[slotProps.index]">
                <div class="flex flex-column align-items-start gap-3">
                    ...
                    <span class="flex flex-wrap align-items-center gap-2">
                        ...
                        <b>Element:</b>
                        <JsonEditorVue v-if="tpaEditionStore.isEditionMode" :class="appThemeStore.isDarkModeOn && 'jse-theme-dark'" v-model="slotProps.data[1].measure.element" @blur="slotProps.data[1].measure.element = parseJsonEditorContent(slotProps.data[1].measure.element)" mode="text" :key="slotProps.data[0]" />
                        <template v-else>
                    ...
                    <span class="flex flex-wrap align-items-center gap-2">
                        ...
                        <small v-if="tpaEditionStore.isEditionMode">(This field is computed automatically from the details)</small>
                    </span>

                    <span class="flex flex-wrap align-items-center gap-2">
                        <i class="pi pi-align-left"></i>
                        <b>Details:</b>
                        <JsonEditorVue v-if="tpaEditionStore.isEditionMode" :class="appThemeStore.isDarkModeOn && 'jse-theme-dark'" v-model="slotProps.data[1].measure.event" @click="currentlyEditingDetails[slotProps.index] = true" @blur="slotProps.data[1].measure.event = parseJsonEditorContent(slotProps.data[1].measure.event); currentlyEditingDetails[slotProps.index] = false" mode="text" :key="slotProps.data[0] + slotProps.index" />
                        <template v-else>
                    ...
                    <Button v-if="tpaEditionStore.isEditionMode" class="mt-2" icon="pi pi-trash" severity="danger" @click="deleteMetric(slotProps.index)" />
