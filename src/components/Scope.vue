...
<template>
  ...
  <div class="flex flex-column gap-2">
    <mark v-if="tpaEditionStore.isEditionMode" class="py-2 mr-3" style="width: fit-content;">
    ...
    <div class="flex align-items-center gap-2">
      <span>Project:</span>
      <EditContent v-if="tpaEditionStore.isEditionMode" :fieldName="fieldName + '.project.default'" :key="fieldName + '.project.default'" />
      <span v-else>{{ tpaEditionStore.getTpaField(fieldName + '.project.default') }}</span>
    </div>

    <div class="flex align-items-center gap-2">
      <span>Course:</span>
      <EditContent v-if="tpaEditionStore.isEditionMode" :fieldName="fieldName + '.class.default'" />
    ...
    <div class="flex gap-2">
      ...
      <Checkbox v-model="isMemberNeeded" :disabled="!tpaEditionStore.isEditionMode" :binary="true" trueValue="*" falseValue="null" @change="tpaEditionStore.updateTpaField(fieldName + '.member.default', isMemberNeeded)" />
    </div>

    <div class="flex align-items-center gap-2">
      <span>Agreement validity start:</span>
      <Calendar v-model="validityInitial" :disabled="!tpaEditionStore.isEditionMode" showIcon showTime :manualInput="false" />
    </div>
