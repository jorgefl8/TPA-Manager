...
async function createClass() {
    if (new_class.value.classId != "" && new_class.value.classId !== null && new_class.value.joinCode != "" && new_class.value.joinCode !== null ) {
        await axios.post(scopesURL, new_class.value, {
            headers: {
    ...
    try {
        ...
        if (validatejsonInput(parsedData)) {
            if (visible_addCredentials.value) {
                new_class.value.credentials.push(parsedData);
            }
            if (visible_addIdentities.value) {
                new_class.value.identities.push(parsedData);
            }
            jsonInput.value = "";
    ...
    <div style="display: grid; justify-items: center;">
        <div class="card ">
            ...
            <div class="content">
                    ...
                    <div class="form">
                        <div class="row">
                            <div class="flex flex-column gap-2" style="width: 45%;">
                                ...
                                <InputText id="class-name" v-model="new_class.classId" />
                            </div>
                            <div class="flex flex-column gap-2" :class="{ 'template-warning': templates.length === 0 }"
                                style="width: 45%; min-width: 135px;">
                                <label for="selected_template">Select TPA template<span class="required-asterisk">*</span></label>
                                <Dropdown id="selected_template" v-model="new_class.templateId" :options="templates"
                                    optionLabel="id" optionValue="id" showClear filter>
                                </Dropdown>
                            </div>
                            <div class="row" v-if="templates.length === 0">
                            <div class="flex flex-column gap-2" style="width: 92%; text-align: center;">
                                <div class="alert alert-danger"
                                    style="color: red; border: 1px solid red; padding: 10px; border-radius: 5px;">
                                    A template must be created in order to create a new course.
                                </div>
                            </div>
                        </div>
                            <div class="flex flex-column gap-2" style="width: 45%;">
                                <label for="join-code">Join code<span class="required-asterisk">*</span></label>
                                <InputText id="join-code" v-model="new_class.joinCode" />
                        ...
                        <div class="row">
                            <div class="flex flex-row gap-2" style="justify-content: center;">
                                ...
                                <InputSwitch v-model="new_class.autoRun" :pt="{
                                    slider: ({ props }) => ({
                            ...
                            <div class="flex flex-row gap-2" style="justify-content: center;">
                                ...
                                <InputSwitch v-model="new_class.hidden" :pt="{
                                    slider: ({ props }) => ({
                                        class: props.modelValue ? 'bg-green-400' : 'bg-gray-300'
                                    })
                                }" />
                ...
                <div class="preview">
                    <pre style="min-width: 205px;">{{ JSON.stringify(new_class, null, 2) }}</pre>
                </div>
            </div>
